 ...
 */
FitManager::~FitManager() { delete m_pConfig; }
...
void FitManager::configure(std::string method) {
  ...
  if (method == "Analytic") {
    ...
    std::cerr << "ERROR: FitManager cannot configure trace viewer"
              << " for unknown fit method " << method << std::endl;
    exit(EXIT_FAILURE);
  }
}
...
void FitManager::readConfigFile() {
  try {
    ...
    std::cerr << "ERROR: failed to configure FitManager -- " << e.what()
              << std::endl;
    exit(EXIT_FAILURE);
  }
}
...
//____________________________________________________________________________
std::vector<double> FitManager::getSinglePulseFit(const DDASFitHit &hit,
...
//____________________________________________________________________________
std::vector<double> FitManager::getDoublePulseFit(const DDASFitHit &hit,
                                                  unsigned low, unsigned high) {
  auto ext = hit.getExtension();
  ;
  auto crate = hit.getCrateID();
  ...
  for (unsigned i = low; i <= high; i++) {
    fit.push_back(
        doublePulse(crate, slot, chan, A1, k1, k2, x1, A2, k3, k4, x2, C, i));
  }
 ...
 */
unsigned FitManager::getLowFitLimit(const DDASFitHit &hit) {
 ...
 * fitting limit is inclusive.
 */
unsigned FitManager::getHighFitLimit(const DDASFitHit &hit) {
  unsigned crate = hit.getCrateID();
  unsigned slot = hit.getSlotID();
  unsigned channel = hit.getChannelID();
  auto limits = m_pConfig->getFitLimits(crate, slot, channel);
 ...
 */
double FitManager::singlePulse(unsigned crate, unsigned slot, unsigned chan,
 ...
 * fit result using the analytic or template fitting functions.
 */
double FitManager::doublePulse(unsigned crate, unsigned slot, unsigned chan,
                               double A1, double k1, double k2, double x1,
 ...
 */
bool FitManager::checkParamValue(double p) {
  if (m_method == ANALYTIC && std::fpclassify(p) == FP_ZERO) {
    std::string msg =
        "Bad parameter value encountered: expected non-zero "
    ...
    std::string msg =
        "Bad parameter value encountered: expected zero "
 ...
 */
void FitManager::issueWarning(std::string msg) {
  QMessageBox msgBox;
