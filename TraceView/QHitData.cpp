...
//____________________________________________________________________________
void QHitData::update(const DDASFitHit &hit) {
  updateHitData(hit);
  if (hit.hasExtension()) {
...
//____________________________________________________________________________
QGroupBox *QHitData::createHitBox() {
  QGroupBox *box = new QGroupBox;
  box->setTitle("Hit Data");

  m_pId = new QLabel(
      QString("Crate: %1 Slot: %2 Channel: %3").arg(0).arg(0).arg(0));
...
//____________________________________________________________________________
QGroupBox *QHitData::createClassifierBox() {
  QGroupBox *box = new QGroupBox;
...
//____________________________________________________________________________
QGroupBox *QHitData::createFitBox() {
  QGroupBox *box = new QGroupBox;
...
//____________________________________________________________________________
void QHitData::createConnections() {
  connect(m_pFitMethod, SIGNAL(currentIndexChanged(int)), this,
          SLOT(configureFit()));
 ...
 * inference code.
 */
void QHitData::updateHitData(const DDASFitHit &hit) {
  // All hits have an ID and hit data:

  QString id = QString("Crate: %1 Slot: %2 Channel: %3")
                   .arg(hit.getCrateID())
                   .arg(hit.getSlotID())
                   .arg(hit.getChannelID());

  // Display 3 decimal places of the double-precision full timestamp
  // possibly with the CFD correction. No limits on the width:

  QString hitData = QString("Energy: %1 Time: %2")
                        .arg(hit.getEnergy())
                        .arg(hit.getTime(), 0, 'f', 3);
...
//____________________________________________________________________________
void QHitData::printFitResults() {
  std::cout << std::endl;
  std::cout << "#######################" << std::endl;
  std::cout << "##### Fit results #####" << std::endl;
  std::cout << "#######################" << std::endl;
  std::cout << "----- Single pulse -----" << std::endl;
  std::cout << std::left << std::setw(6) << "Iter: " << std::setw(2)
            << m_pExtension->onePulseFit.iterations << std::setw(9)
            << " Status: " << std::setw(2)
            << m_pExtension->onePulseFit.fitStatus << std::setw(8)
            << " Chisq: " << std::setw(6) << m_pExtension->onePulseFit.chiSquare
            << std::setw(9) << " Offset: " << std::setw(8)
            << m_pExtension->onePulseFit.offset << std::endl;

  std::cout << std::left << std::setw(5) << "Amp: " << std::setw(8)
            << m_pExtension->onePulseFit.pulse.amplitude << std::setw(8)
            << " Steep: " << std::setw(8)
            << m_pExtension->onePulseFit.pulse.steepness << std::setw(8)
            << " Decay: " << std::setw(8)
            << m_pExtension->onePulseFit.pulse.decayTime << std::setw(6)
            << " Pos: " << std::setw(8)
            << m_pExtension->onePulseFit.pulse.position << std::endl;

  std::cout << std::endl;
  std::cout << "----- Double pulse -----" << std::endl;
  std::cout << std::left << std::setw(6) << "Iter: " << std::setw(2)
            << m_pExtension->twoPulseFit.iterations << std::setw(9)
            << " Status: " << std::setw(2)
            << m_pExtension->twoPulseFit.fitStatus << std::setw(8)
            << " Chisq: " << std::setw(6) << m_pExtension->twoPulseFit.chiSquare
            << std::setw(9) << " Offset: " << std::setw(8)
            << m_pExtension->twoPulseFit.offset << std::endl;

  std::cout << std::left << std::setw(5) << "Amp1: " << std::setw(8)
            << m_pExtension->twoPulseFit.pulses[0].amplitude << std::setw(8)
            << " Steep1: " << std::setw(8)
            << m_pExtension->twoPulseFit.pulses[0].steepness << std::setw(8)
            << " Decay1: " << std::setw(8)
            << m_pExtension->twoPulseFit.pulses[0].decayTime << std::setw(6)
            << " Pos1: " << std::setw(8)
            << m_pExtension->twoPulseFit.pulses[0].position << std::endl;

  std::cout << std::left << std::setw(5) << "Amp2: " << std::setw(8)
            << m_pExtension->twoPulseFit.pulses[1].amplitude << std::setw(8)
            << " Steep2: " << std::setw(8)
            << m_pExtension->twoPulseFit.pulses[1].steepness << std::setw(8)
            << " Decay2: " << std::setw(8)
            << m_pExtension->twoPulseFit.pulses[1].decayTime << std::setw(6)
            << " Pos2: " << std::setw(8)
            << m_pExtension->twoPulseFit.pulses[1].position << std::endl;

  std::cout << std::endl;
  std::cout << "----- Classification -----" << std::endl;
  std::cout << std::left << std::setw(5) << "P(single): " << std::setw(8)
            << m_pExtension->singleProb << "\n"
            << std::setw(5) << "P(double): " << std::setw(8)
            << m_pExtension->doubleProb << std::endl;
}
