...
QTraceView::QTraceView(QCommandLineParser &parser, QWidget *parent)
    : QWidget(parent), m_pDecoder(nullptr), m_pFitManager(nullptr),
      ...
      m_pExitAction(nullptr), m_pMainButtons{nullptr},
      m_pSelectButtons{nullptr}, m_pSelectLineEdit(nullptr),
      m_pHitFilter{nullptr}, m_pTopBoxes(nullptr), m_pHitData(nullptr),
      m_pHitSelectList(nullptr), m_pPlotWidget(nullptr), m_pRootCanvas(nullptr),
      m_pTimer(nullptr), m_pStatusBar(nullptr) {
  m_pTimer = new QTimer;
...
void QTraceView::getNextEvent() {
  ...
  std::string msg = m_pDecoder->getFilePath() + " -- Event " +
                    std::to_string(m_pDecoder->getEventIndex());
  setStatusBar(msg);
}
 ...
 */
void QTraceView::getNextEventWithTraces() {
...
//____________________________________________________________________________
void QTraceView::getNextEventFromList() {
...
//____________________________________________________________________________
void QTraceView::handleRootEvents() { gSystem->ProcessEvents(); }

//____________________________________________________________________________
void QTraceView::test() {
  std::time_t result = std::time(nullptr);
  std::cout << "Test slot call at: " << std::asctime(std::localtime(&result))
            << std::endl;
}
 ...
 */
void QTraceView::createTopBoxWidget() {
...
//____________________________________________________________________________
void QTraceView::createHitSelectWidget() {
 ...
 */
void QTraceView::createPlotWidget() {
 ...
 */
void QTraceView::createConnections(bool useEvtList) {
  // Skip button
  ...
  // Next button.
  if (useEvtList) {
    connect(m_pMainButtons[0], SIGNAL(clicked()), this,
            SLOT(getNextEventFromList()));
  } else {
    connect(m_pMainButtons[0], SIGNAL(clicked()), this,
            SLOT(getNextEventWithTraces()));
  }

  // Update buttons
  connect(m_pMainButtons[1], SIGNAL(clicked()), this, SLOT(filterHits()));
  connect(m_pMainButtons[1], SIGNAL(clicked()), this,
          SLOT(updateSelectableHits()));
 ...
 */
void QTraceView::configureSource(QString filename) {
...
//____________________________________________________________________________
void QTraceView::skipAndSelect(int count) {
 ...
 */
void QTraceView::selectEventByIndex(int idx) {
  ...
  if (current >= 0 && skipCount < 0) {
    issueWarning("Current event = " + std::to_string(current) +
 ...
 */
bool QTraceView::isValidHit(const DDASFitHit &hit) {
...
//____________________________________________________________________________
void QTraceView::setStatusBar(std::string msg) {
  m_pStatusBar->showMessage(tr(msg.c_str()));
}

//____________________________________________________________________________
void QTraceView::reset() {
  m_hits.clear();
  m_filteredHits.clear();
  updateSelectableHits();
  m_pRootCanvas->clear();
...
//____________________________________________________________________________
void QTraceView::enableGUI(bool status) {
  enableMainGUI(status);
  enableSelectGUI(status);
}

//____________________________________________________________________________
void QTraceView::enableMainGUI(bool status) {
  for (int i = 0; i < 2; i++) {
...
//____________________________________________________________________________
void QTraceView::enableSelectGUI(bool status) {
  for (int i = 0; i < 2; i++) {
 ...
 */
void QTraceView::issueWarning(std::string msg) {
  QMessageBox msgBox;
 ...
 */
void QTraceView::issueEOFWarning() {
  m_hits.clear();
  m_filteredHits.clear();
  updateSelectableHits();
  m_pRootCanvas->clear();
 ...
 */
void QTraceView::parseArgs(QCommandLineParser &parser) {
 ...
 */
void QTraceView::parseEventList(QString fname) {
  std::ifstream fin;
  ...
  while (fin >> evt) {
    if (evt < 0) {
      std::cerr << "Negative event index " << evt
                << " found when parsing event file " << fname.toStdString()
                << "! Ignoring..." << std::endl;
    } else {
      tmp.push_back(evt);
    }
