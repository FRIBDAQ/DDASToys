...
DDASDecoder::DDASDecoder()
    : m_pSourceURL(nullptr), m_pSource(nullptr), m_pProcessor(nullptr),
      m_count(0) {
...
void DDASDecoder::createDataSource(std::string src) {
  ...
  std::cout << "Filename in DDASDecoder: " << m_pSourceURL->getPath()
            << std::endl;
 ...
 *
 */
CRingItem *DDASDecoder::getNextPhysicsEvent() {
  CRingItem *pItem;
  while (true) {
 ...
 */
void DDASDecoder::processRingItem(CRingItem &item) {
