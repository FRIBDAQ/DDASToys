...
//____________________________________________________________________________
void QRootCanvas::drawHit(const DDASFitHit &hit) {
  drawTrace(hit);
 ...
 */
void QRootCanvas::drawTrace(const DDASFitHit &hit) {
 ...
 */
void QRootCanvas::drawSingleFit(const DDASFitHit &hit) {
 ...
 * be checked by the caller using hit.hasExtension().
 */
void QRootCanvas::drawDoubleFit(const DDASFitHit &hit) {
  unsigned low = m_pFitManager->getLowFitLimit(hit);
  unsigned high = m_pFitManager->getHighFitLimit(hit);
  unsigned fitRange = high - low + 1; // +1 since range is [low, high]
...
//____________________________________________________________________________
void QRootCanvas::drawFitLegend() {
  if (!m_pFitLegend) {
