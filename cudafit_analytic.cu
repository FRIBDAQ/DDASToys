 ...
 */
static void reportCudaError(const char *context) {
 ...
 */
static unsigned traceToGPU(std::vector<uint16_t> trace,
  ...
  if (result > MAXPOINTS) {
    throw std::runtime_error(
        "CUDA fit: reduced trace has " + std::to_string(result) +
        " points, exceeds MAXPOINTS=" + std::to_string(MAXPOINTS) +
        ". Use a shorter trace or a non-CUDA fitter.");
  }
  ...
  if (cudaMalloc(&d_xCoords, xcoords.size() * sizeof(unsigned short)) !=
      cudaSuccess) {
    reportCudaError("Allocating GPU memory for trace x-coordinates");
  }
  if (cudaMalloc(&d_yCoords, ycoords.size() * sizeof(unsigned short)) !=
      cudaSuccess) {
    reportCudaError("Allocating GPU memory for trace y-coordinates");
  }

  if (cudaMemcpy(d_xCoords, xcoords.data(),
                 xcoords.size() * sizeof(unsigned short),
                 cudaMemcpyHostToDevice) != cudaSuccess) {
    reportCudaError("Moving trace x coordinates into the GPU");
  }
  if (cudaMemcpy(d_yCoords, ycoords.data(),
                 ycoords.size() * sizeof(unsigned short),
                 cudaMemcpyHostToDevice) != cudaSuccess) {
  ...
  if (cudaMemcpy(d_pWeights, h_pWeights, result * sizeof(float),
                 cudaMemcpyHostToDevice) != cudaSuccess) {
 ...
 */
__host__ __device__ float singlePulse(float A1, float k1, float k2, float x1,
                                      float C, float x) {
 ...
 */
__host__ __device__ float doublePulse(float A1, float k1, float k2, float x1,
 ...
 */
__host__ __device__ float chiFitness1(const float *pParams, float x, float y,
                                      float wt) {
 ...
 * @return Chi-square between fit function and trace data.
 */
__host__ __device__ float chiFitness2(const float *pParams, float x, float y,
                                      float wt) {
  // Get the parameters from the fit:
  float a1 = pParams[A1];
  float a2 = pParams[A2];
...
__global__ void d_fitness1(const float *pSolutions, float *pFitnesses,
                           int nParams, int solStride, int nSol, unsigned short *pXcoords,
...
void h_fitSingle(const CudaOptimize::SolutionSet *solutions,
  ...
  d_fitness1<<<grid, myBlockSize, n_tracePoints * sizeof(float)>>>(
      d_solutions, d_fitnesses, P1_NPARAMS, solStride, nsol, d_xCoords, d_yCoords,
...
void h_fitDouble(const CudaOptimize::SolutionSet *solutions,
  ...
  d_fitness2<<<grid, myBlockSize, n_tracePoints * sizeof(float)>>>(
      d_solutions, d_fitnesses, P2_NPARAMS, solStride, nsol, d_xCoords, d_yCoords,
      d_pWeights, n_tracePoints);
...
void ddastoys::analyticfit::cudafit1(
	...
	opt->setTerminationFlags((CudaOptimize::TERMINATION_FLAGS)(
