 ...
 */
__device__ static float logistic(float A, float k, float x1, float x) {
 ...
 */
__device__ static float decay(float A, float k, float x1, float x) {
 ...
 */
__device__ static float singlePulse(float A1, float k1, float k2, float x1,
                                    float C, float x) {
 ...
 */
__device__ static float doublePulse(float A1, float k1, float k2, float x1,
 ...
 */
__device__ static float dp1dA(float k1, float k2, float x1, float x, float w,
 ...
 * @return Value of (dP1/dA)(x)/w
 */
__device__ static double dp1dAd(double k1, double k2, double x1, double x,
 ...
 */
__device__ static float dp1dk1(float A, float k1, float k2, float x1, float x,
                               float w, float erise, float efall) {
 ...
 * @return Value of (dP1/dk1)(x)/w
 */
__device__ static double dp1dk1d(double A, double k1, double k2, double x1,
                                 double x, double w, double erise,
                                 double efall) {
 ...
 */
__device__ static float dp1dk2(float A, float k1, float k2, float x1, float x,
                               float w, float erise, float efall) {
  float d1 = A * efall; // decay(A, k2, x1, x);
  float num = d1 * (x1 - x);
  float l = 1.0 / (1.0 + erise); // logistic(1.0, k1, x1, x);
 ...
 * @return   Value of (dP1/dk2)(x)/w.
 */
__device__ static double dp1dk2d(double A, double k1, double k2, double x1,
                                 double x, double w, double erise,
                                 double efall) {
  double d1 = A * efall; // decay(A, k2, x1, x);
  double num = d1 * (x1 - x);
 ...
 * @return Value of (dP1/dk2)(x)/w.
 */
__device__ static float dp1dx1(float A, float k1, float k2, float x1, float x,
                               float w, float erise, float efall) {
 ...
 * @return Value of (dP1/dk2)(x)/w.
 */
__device__ static double dp1dx1d(double A, double k1, double k2, double x1,
                                 double x, double w, double erise,
                                 double efall) {
 ...
 */
__device__ static float dp1dC(float A, float k1, float k2, float x1, float x,
                              float w) {
  return 1.0 / w;
}
 ...
 * @param x1  Position.
 */
__global__ void residual1(void *tx, void *ty, void *res, unsigned len, float C,
 ...
 * @param x1  Position.
 */
__global__ void jacobian1(void *tx, void *ty, void *jac, unsigned len, float A,
                          float k1, float k2, float x1) {
  int i = blockIdx.x * blockDim.x + threadIdx.x;
  if (i < len) {
    unsigned short *tracex = static_cast<unsigned short *>(tx);
...
ddastoys::analyticfit::CudaFitEngine1::CudaFitEngine1(
  ...
  if (cudaMemcpy(m_dXtrace, x, m_npts * sizeof(unsigned short),
                 cudaMemcpyHostToDevice) != cudaSuccess) {
    throwCudaError("Failed to move trace x coords -> gpu");
  }
  if (cudaMemcpy(m_dYtrace, y, m_npts * sizeof(unsigned short),
                 cudaMemcpyHostToDevice) != cudaSuccess) {
...
void ddastoys::analyticfit::CudaFitEngine1::jacobian(const gsl_vector *p,
  ...
  jacobian1<<<(m_npts + 31) / 32, 32>>>(m_dXtrace, m_dYtrace, m_dJacobian,
                                        m_npts, A, k1, k2, x1);
  ...
  if (cudaMemcpy(Jac, m_dJacobian, m_npts * 5 * sizeof(float),
...
void ddastoys::analyticfit::CudaFitEngine1::residuals(const gsl_vector *p,
  ...
  residual1<<<(m_npts + 31) / 32, 32>>>(m_dXtrace, m_dYtrace, m_dResiduals,
                                        m_npts, C, A, k1, k2, x1);
  ...
  float resids[m_npts];
  if (cudaMemcpy(resids, m_dResiduals, m_npts * sizeof(float),
                 cudaMemcpyDeviceToHost) != cudaSuccess) {
 ...
 */
void ddastoys::analyticfit::CudaFitEngine1::throwCudaError(const char *msg) {
  std::string e = "Error: ";
  e += msg;
  e += " : ";
 ...
 */
__global__ void residual2(void *xtc, void *ytc, void *res, unsigned npts,
                          float C, float A1, float k11, float k12, float x1,
 ...
 */
__global__ void jacobian2(void *xtc, void *jac, unsigned npts, double A1,
                          double k1, double k2, double x1, double A2, double k3,
  ...
  if (i < npts) {
    ...
    // Pulse 1 elements:
    j[k] = dp1dAd(k1, k2, x1, x, 1.0, erise1, efall1);
    k += npts;
    j[k] = dp1dk1d(A1, k1, k2, x1, x, 1.0, erise1, efall1);
    k += npts;
    j[k] = dp1dk2d(A1, k1, k2, x1, x, 1.0, erise1, efall1);
    k += npts;
    j[k] = dp1dx1d(A1, k1, k2, x1, x, 1.0, erise1, efall1);
    k += npts;

    // Pulse 2 elements:
    j[k] = dp1dAd(k3, k4, x2, x, 1.0, erise2, efall2);
    k += npts;
    j[k] = dp1dk1d(A2, k3, k4, x2, x, 1.0, erise2, efall2);
    k += npts;
    j[k] = dp1dk2d(A2, k3, k4, x2, x, 1.0, erise2, efall2);
    k += npts;
    j[k] = dp1dx1d(A2, k3, k4, x2, x, 1.0, erise2, efall2);
    k += npts;
...
void ddastoys::analyticfit::CudaFitEngine2::jacobian(const gsl_vector *p,
  ...
  jacobian2<<<(m_npts + 31) / 32, 32>>>(m_dXtrace, m_dJacobian, m_npts, A1, k1,
                                        k2, x1, A2, k3, k4, x2, C);
  ...
  double jac[m_npts * 9];
  if (cudaMemcpy(jac, m_dJacobian, m_npts * 9 * sizeof(double),
                 cudaMemcpyDeviceToHost) != cudaSuccess) {
  ...
  for (int i = 0; i < m_npts; i++) {
    int k = i;
    gsl_matrix_set(J, i, 0, jac[k]);
    k += m_npts;
    gsl_matrix_set(J, i, 1, jac[k]);
    k += m_npts;
    gsl_matrix_set(J, i, 2, jac[k]);
    k += m_npts;
    gsl_matrix_set(J, i, 3, jac[k]);
    k += m_npts;
    gsl_matrix_set(J, i, 4, jac[k]);
    k += m_npts;
    gsl_matrix_set(J, i, 5, jac[k]);
    k += m_npts;
    gsl_matrix_set(J, i, 6, jac[k]);
    k += m_npts;
    gsl_matrix_set(J, i, 7, jac[k]);
    k += m_npts;
    gsl_matrix_set(J, i, 8, jac[k]);
    k += m_npts;
  }
}
...
void ddastoys::analyticfit::CudaFitEngine2::residuals(const gsl_vector *p,
  ...
  residual2<<<(m_npts + 31) / 32, 32>>>(m_dXtrace, m_dYtrace, m_dResiduals,
                                        m_npts, C, A1, k1, k2, x1, A2, k3, k4,
                                        x2);
  ...
  if (cudaMemcpy(residuals, m_dResiduals, m_npts * sizeof(float),
                 cudaMemcpyDeviceToHost) != cudaSuccess) {
 ...
 */
void ddastoys::analyticfit::CudaFitEngine2::throwCudaError(const char *msg) {
  std::string e = "Error: ";
  e += msg;
  e += " : ";
