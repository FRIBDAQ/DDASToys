 ...
 */
FitEditorTemplate::~FitEditorTemplate() { delete m_pConfig; }
...
FitEditorTemplate::operator()(pRingItemHeader pHdr, pBodyHeader pBHdr,
  ...
  if (m_pConfig->fitChannel(crate, slot, chan)) {
    ...
    if (trace.size() > 0) { // Need a trace to fit
      ...
      auto alignPoint = m_pConfig->getTemplateAlignPoint(crate, slot, chan);
      ...
      if (classification) {
        ...
        if (classification & 1) {
          ...
          Timer timer;
#endif
          templatefit::lmfit1(&(pFit->s_extension.onePulseFit), trace,
        ...
        if (classification & 2) {
          ...
          if (classification & 1) {
            ...
            Timer timer;
#endif
            templatefit::lmfit2(&(pFit->s_extension.twoPulseFit), trace,
...
int FitEditorTemplate::pulseCount(DDASHit &hit) {
