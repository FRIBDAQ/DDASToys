 ...
 */
static void reduceTrace(std::vector<std::pair<uint16_t, uint16_t>> &points,
 ...
 */
static int gsl_p1Residuals(const gsl_vector *p, void *pData, gsl_vector *r) {
 ...
 * @note GPU Implementation hint: This function is nicely data parallel.
 */
static int gsl_p1Jacobian(const gsl_vector *p, void *pData, gsl_matrix *J) {
 ...
 */
static int gsl_p1Compute(const gsl_vector *p, void *pData, gsl_vector *resids,
                         gsl_matrix *J) {
  CFitEngine *pEngine = reinterpret_cast<CFitEngine *>(pData);
 ...
 */
static double estimateK2(int x0, double C0,
                         const std::vector<uint16_t> &trace) {
  ...
  for (size_t i = x0; i < trace.size(); i++) {
    ...
    if ((khalf < 0.0) && (v < 0.5 * maxval)) {
      khalf = ln2 / (i - x0);
      break; // Note that we've got both so we can quit.
    }
  }
  ...
  if ((k34 > 0) && (khalf > 0))
    return 0.5 * (k34 + khalf); // Average of both.
  if (k34 > 0)
    return k34;       // Only have k34.
 ...
 */
static int gsl_p2Residuals(const gsl_vector *p, void *pData, gsl_vector *r) {
  CFitEngine *pEngine = reinterpret_cast<CFitEngine *>(pData);
  pEngine->residuals(p, r);
 ...
 */
static int gsl_p2Jacobian(const gsl_vector *p, void *pData, gsl_matrix *j) {
 ...
 */
static int gsl_p2Compute(const gsl_vector *p, void *pData, gsl_vector *resids,
                         gsl_matrix *J) {
  CFitEngine *pEngine = reinterpret_cast<CFitEngine *>(pData);
  pEngine->residuals(p, resids);
  pEngine->jacobian(p, J);
...
void ddastoys::analyticfit::lmfit2(fit2Info *pResult,
  ...
  if (!pSinglePulseFit) {
    ...
    fit1 = *pSinglePulseFit; // Use what's passed in if possible.
  }
  ...
  double ChiSquare = chiSquare2(A1, K1, K2, X1, A2, K3, K4, X2, C, points);
 ...
 * @return GSL_SUCCESS can't really fail.
 */
static int gsl_p2ftResiduals(const gsl_vector *p, void *pData, gsl_vector *r) {
 ...
 */
static int gsl_p2ftJacobian(const gsl_vector *p, void *pData, gsl_matrix *j) {
  ...
  for (size_t i = 0; i < points.size(); i++) {
    ...
    gsl_matrix_set(j, i, P2FTA1_INDEX,
                   dp1dA(k1, k2, x1, x, 1.0, erise1, efall1));
    gsl_matrix_set(j, i, P2FTK1_INDEX,
                   dp1dk1(A1, k1, k2, x1, x, 1.0, erise1, efall1) +
                       dp1dk1(A2, k1, k2, x2, x, 1.0, erise2, efall2));
    gsl_matrix_set(j, i, P2FTK2_INDEX,
                   dp1dk2(A1, k1, k2, x1, x, 1.0, erise1, efall1) +
                       dp1dk2(A2, k1, k2, x2, x, 1.0, erise2, efall2));
    gsl_matrix_set(j, i, P2FTX1_INDEX,
                   dp1dx1(A1, k1, k2, x1, x, 1.0, erise1, efall1));

    // For pulse 2 elements:  A1->A2,  x1 -> x2:

    gsl_matrix_set(j, i, P2FTA2_INDEX,
                   dp1dA(k3, k4, x2, x, 1.0, erise2, efall2));
    gsl_matrix_set(j, i, P2FTX2_INDEX,
                   dp1dx1(A2, k3, k4, x2, x, 1.0, erise2, efall2));
 ...
 * @result GSL_SUCCESS can only be that.
 */
static int gsl_p2ftCompute(const gsl_vector *p, void *pData, gsl_vector *resids,
                           gsl_matrix *J) {
 ...
 * by chi-square minimization.
 */
void ddastoys::analyticfit::lmfit2fixedT(
    fit2Info *pResult, std::vector<uint16_t> &trace,
    const std::pair<unsigned, unsigned> &limits, fit1Info *pSinglePulseFit,
    uint16_t saturation) {
  unsigned low = limits.first;
  unsigned high = limits.second;
  ...
  if (solver == nullptr) {
    throw std::runtime_error(
        "lmfit2fixed Unable to allocate fit solver workspace");
  }
