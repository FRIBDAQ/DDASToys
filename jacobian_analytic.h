 ...
 * by the actual fit.
 *
 * @todo (ASC 10/30/23): Manages some device pointers. Not copyable, and most 
 * likely is not ever copied. But we can make it safe. 
 */
...
public:
    ...
    virtual void jacobian(const gsl_vector* p, gsl_matrix *J);
    /**
     * @brief Triggers a pointwise parallel residual kernel in the device and 
     * impedance matches that with GSL's requirements.
     *  @param p Parameter vector.
...
public:
    ...
    ~CudaFitEngine2();
    /**
     * @brief Marshall the parameter and call the jacobian2 kernel. Then pull 
     * the Jacobian matrix out of the GPU and marshall it back into the GSL 
     * Jacobian matrix.
