 ...
 */
static std::tuple<TT, TT, TT> preprocessInput(std::vector<uint16_t> &trace,
                                              unsigned nsamples) {
  ...
  TT input = torch::empty({static_cast<long>(trace.size())}, torch::kFloat32);
  float *pData = input.data_ptr<float>(); // Points to first element
  std::transform(trace.begin(), trace.end(), pData,
                 [](const uint16_t &data) { return static_cast<float>(data); });
 ...
 */
static void postProcessOutput(FitInfo *pResult, c10::IValue &output,
                              double xscale, double yscale, double offset) {
 ...
 * overhead.
 */
void ddastoys::mlinference::performInference(
