...
/**
 * Compute the Jacobian matrix  partial derivatives evaluated at each
 * data point.
 *
...
/**
 * Compute function calls gsl_p1Residuals and gsl_p1Jacobian.
 *
 * @param[in]  p       Current fit parameterization.
 ...
 *
 * @result int  GSL_SUCCESS, I don't see how this can fail.
 *
 */
static int
...
/**
 * Compute function for double pulse fits which evaluates the Jacobian matrix 
 * and calculates the fit residuals.
 *
 ...
 *                         guesses if present. Otherwise a single pulse fit 
 *                         is done for that.
...
/**
 * Compute method for double pulse fits that calls the functions to both
...
/**
 * @brief Driver for the GSL LM fitter for double pulses, constraining the two
 * timing parameters (rise time and fall time) to be the same for both pulses.
 *
 * @param pResult   Results will be stored here.
 * @param trace     References the trace to fit.
 * @param limits    The limits of the trace that can be fit.
 ...
 *                         guesses if present. Otherwise a single pulse fit 
 *                         is done for that.
 * @param saturation  ADC saturation value. Points with values at or above 
 *                    this value are removed from the fit.
 */
void
