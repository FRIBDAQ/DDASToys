...
void ddastoys::templatefit::lmfit1(fit1Info *pResult,
  ...
  solver = gsl_multifit_nlinear_alloc(method, &function_params, npts,
                                      P1_PARAM_COUNT);
  if (!solver) {
  ...
  status = gsl_multifit_nlinear_driver(SINGLE_MAXITERS, xtol, gtol, ftol,
...
void ddastoys::templatefit::lmfit2(fit2Info *pResult,
  ...
  status = gsl_multifit_nlinear_driver(DOUBLE_MAXITERS, xtol, gtol, ftol,
                                       nullptr, nullptr, &info, solver);
