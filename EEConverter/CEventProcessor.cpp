...
CEventProcessor::CEventProcessor()
    : m_pDataSource(nullptr), m_pProcessor(nullptr), m_itemCount(0),
      m_skipCount(0) {}
 ...
 */
int CEventProcessor::operator()(int argc, char *argv[]) {
  // Parse arguments:
  ...
  std::cout << "Filename in CEventProcessor: " << m_pDataSource->getPath()
            << std::endl;
  ...
  if (parse.skip_given) {
    if (parse.skip_arg < 0) {
      ...
      msg << "--skip value must be >= 0 but is " << parse.skip_arg << std::endl;
  ...
  if (parse.count_given) {
    if (parse.count_arg < 0) {
      std::stringstream msg;
      msg << "--count value must be >= 0 but is " << parse.count_arg
          << std::endl;
      throw std::invalid_argument(msg.str());
    } else {
 ...
 */
void CEventProcessor::processRingItem(CRingItem &item) {
